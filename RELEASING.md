...
To release a new version of `cardano-keys`, use the [Actions > Release](https://github.com/intersectmbo/cardano-keys/actions/workflows/release.yml) workflow (or via CLI):

```bash
gh workflow run release.yml -f package=cardano-keys
```

To override the auto-computed version:
```bash
gh workflow run release.yml -f package=cardano-keys -f version=11.1.0.0
```
...
```bash
nix run github:input-output-hk/cardano-dev#herald -- next cardano-keys
```

1. Create new branch e.g. `release/cardano-keys-11.1.0.0`:
```bash
git checkout -b release/cardano-keys-11.1.0.0
```
...
1.  Run the following script, replacing `<commit-hash>` with the just tagged commit hash:
    ```bash
    ./scripts/add-from-github.sh https://github.com/IntersectMBO/cardano-keys <commit-hash> cardano-keys
    ```
...
1. Merge the PR - you don't need additional approvals for that if you belong to the correct GitHub access group.
    ...
    After package gets released, you can check the released version at: https://chap.intersectmbo.org/all-package-versions/ and update the version in the dependant packages, in their cabal files, for example: `cardano-keys ^>= 11.0`
...
1. If the release pipeline (if any, see e.g. [here for CLI](https://github.com/IntersectMBO/cardano-cli/actions/workflows/release-upload.yaml)) fails
   ...
   doesn't matter much): `gh workflow run "Release Upload" -r $(git branch --show-current) -f target_tag=cardano-keys-11.0.0.0`.
1. If a GitHub release is automatically created by the CI, as visible on https://github.com/IntersectMBO/cardano-keys/releases,
...
1. For each version we decide to patch, create a branch starting from the tag of the unpatched version. For example, for `11.0.2.0`, we create a new branch `release/cardano-keys-11.0.2.0` based on the tag `cardano-keys-11.0.1.0`.

1. From each of the release branches, create a dedicated branch for the backported fix, such as `backport/cardano-keys-11.0/fix-bug`. Cherry-pick the bug fix onto this backport branch, resolving any conflicts that arise. And issue and merge a Pull Request (PR) from the backport branch to the patch release branch (e.g., `release/cardano-keys-11.0.2.0`).
