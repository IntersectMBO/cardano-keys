...
# resolve on the published docs site — we only host this repository's own
...
# local rebuilds), and readers of these docs rarely click into
...
#          Our haddocks are built from master, which pins
...
# (cardano-keys-11.0.0.0-inplace -> cardano-keys)
symlink_count=0
for dir in "$WEBSITE_DIR"/*/; do
  name="$(basename "$dir")"
...
while IFS= read -r pkg; do
  ...
  if [[ "$url" == "HACKAGE" ]]; then
    ...
    # these docs, and Haddock's per-module URL structure doesn't line
...
if [[ $reexport_total -gt 0 ]]; then
  ...
  # for local-only types (e.g. this package's own types) is left
  # untouched.
